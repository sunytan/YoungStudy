...
package ty.youngstudy.com.ui.view.layout;
...
public class BalloonRelativeLayout extends RelativeLayout {
    ...
    private void init() {//初始化显示的图片
        ...
        drawables[2] = mBalloon3;//设置view宽高相等，默认都是50dp

        layoutParams = new LayoutParams(mViewHeight, mViewHeight);
        layoutParams.addRule(ALIGN_PARENT_BOTTOM, TRUE);// 初始化插值器
    ...
    public void addBalloon() {
        final ImageView imageView = new ImageView(getContext());//随机选一个
        ...
        animator.addListener(new AnimatorListenerAdapter() {
            ...
            public void onAnimationEnd(Animator animation) {
                super.onAnimationEnd(animation);//view动画结束后remove掉
    ...
    private ValueAnimator getBezierValueAnimator(final View target) {//初始化一个自定义的贝塞尔曲线插值器，并且传入控制点
        BezierEvaluator evaluator = new BezierEvaluator(getPointF(), getPointF());//传入了曲线起点（左下角）和终点（顶部随机0）
        ...
        animator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            @Override
            public void onAnimationUpdate(ValueAnimator animation) {//获取到贝塞尔曲线轨迹上的x和y值 赋值给view
    ...
    class BezierEvaluator implements TypeEvaluator<PointF> {
        ...
        public PointF evaluate(float time, PointF startValue, PointF endValue) {
            ...
            PointF point = new PointF();//这么复杂的公式让我计算真心头疼，但是计算机很easy
