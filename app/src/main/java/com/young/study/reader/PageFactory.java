...
//    private MappedByteBuffer mbBuff;
    ...
    public PageFactory(Context context, int width, int height, int fontSize, String bookId,
        ...
        mTitlePaint.setColor(ContextCompat.getColor(MyApplication.getInstance().getContext(), R.color.chapter_title_day));
...
//        File file = FileUtils.getChapterFile(bookId, chapter);
    	File file = NovelFileUtils.getChapterFile(bookId, chapter);
    ...
    public int openBook(int chapter, int[] position) {
        ...
        try {
            ...
            if (length > 10) {
                ...
                //for over limit bug,lw
                if(curBeginPos >= length || curEndPos > length) {
                	curBeginPos = 0;
                	curEndPos = 0;
                }
...
//        mbBufferLen = "downloading".getBytes().length;
    ...
    private void pageUp() {
        String strParagraph = "";
        Vector<String> lines = new Vector<String>(); // 页面行
        int paraSpace = 0;
        mPageLineCount = mVisibleHeight / (mFontSize + mLineSpace);
        while ((lines.size() < mPageLineCount) && (curBeginPos > 0)) {
            Vector<String> paraLines = new Vector<String>(); // 段落行
...
//        return (currentChapter < chaptersList.size()&&FileUtil.isChapterExist(currentChapter + 1)) || curEndPos < mbBufferLen;
//    }
//
//    public boolean hasPrePage() {
//        return (currentChapter > 1&& (FileUtil.isChapterExist(currentChapter - 1) || curBeginPos > 0)) || (currentChapter == 1 && curBeginPos > 0);
//    }
